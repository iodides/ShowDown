...
public class Torrentube {
    ...
    private final static Logger log = Logger.getLogger(CrawlerThread.class);
    ...
    private boolean getBaseUrl() {
        ...
        try {
            baseUrl = DB.getTorrentSiteUrl(siteName);
            if(!baseUrl.equals("")) result = true;
		...
		for(int i=1; i<=max_page; ) {
	        ...
	        try {
				...
				for(int j=0; j<array.size(); j++) {
                    ...
                    if(DB.insertTorrent(siteName, id, title, url, magnet)) {
                        log.info("토렌트 추가 : "+ title);
                        insertCnt++;
                    }
				}
